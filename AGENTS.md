...
An explicitly reduced `demo` workflow is also implemented for presentations. It reuses readiness and isolated-worktree preparation, runs a separate Codex worker with native goals disabled, then commits, pushes, and creates a pull request. It intentionally skips externally managed deterministic checks and independent review, and its pull-request body records that assurance boundary. `IMPLEMENTER_WORKFLOW=demo` selects it for webhook-driven runs; production remains the default. Never present a demo pull request as equivalent to a production result.
